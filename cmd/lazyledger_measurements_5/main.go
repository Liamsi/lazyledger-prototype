...
func generateSimpleBlock(registrarTxes int, otherTxes, txSize int) (*lazyledger.SimpleBlock, [namespaceSize]byte, [namespaceSize]byte) {
	...
	copy(rns1[:], []byte("reg1"))
	registrarApp.SetNamespace(rns1)
	b.RegisterApplication(registrarApp)
	sb.AddMessage(currencyApp.GenerateTransaction(privA, pubB, 100000, nil))

	for i := 0; i < registrarTxes; i++ {
		name := make([]byte, 8)
		rand.Read(name)
		sb.AddMessage(registrarApp.GenerateTransaction(privA, name))
	}

	ms3 := lazyledger.NewSimpleMap()
	registrarApp2 := lazyledger.NewRegistrar(ms3, currencyApp, pubCBytes)
	var rns2 [namespaceSize]byte
	copy(rns2[:], []byte("reg2"))
	registrarApp2.SetNamespace(rns2)
	b.RegisterApplication(registrarApp2)
	sb.AddMessage(currencyApp.GenerateTransaction(privA, pubC, 100000, nil))

	for i := 0; i < otherTxes; i++ {
		name := make([]byte, 8)
		rand.Read(name)
		sb.AddMessage(registrarApp2.GenerateTransaction(privA, name))
	}
...
func generateProbabilisticBlock(registrarTxes int, otherTxes, txSize int) (*lazyledger.ProbabilisticBlock, [namespaceSize]byte, [namespaceSize]byte) {
	...
	registrarApp.SetNamespace(rns1)
	b.RegisterApplication(registrarApp)
	pb.AddMessage(currencyApp.GenerateTransaction(privA, pubB, 100000, nil))

	for i := 0; i < registrarTxes; i++ {
		name := make([]byte, 8)
		rand.Read(name)
		pb.AddMessage(registrarApp.GenerateTransaction(privA, name))
	}
	...
	registrarApp2.SetNamespace(rns2)
	b.RegisterApplication(registrarApp2)
	pb.AddMessage(currencyApp.GenerateTransaction(privA, pubC, 100000, nil))

	for i := 0; i < otherTxes; i++ {
		name := make([]byte, 8)
		rand.Read(name)
		pb.AddMessage(registrarApp2.GenerateTransaction(privA, name))
	}
